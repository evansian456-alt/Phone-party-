...
// Body size limits
// Skip JSON parsing for Stripe webhook paths — they need the raw body for signature verification.
// The per-route express.raw() middleware on those routes handles their own body parsing.
...
// Duplicate GET /api/sync/metrics removed — canonical version is defined below near line 3110.
...
// Admin metrics dashboard (secured with requireAdmin – JWT + isAdmin flag)
 ...
 */
app.get('/api/admin/moderation/reports', rateLimit({ windowMs: 60000, max: 60 }), authMiddleware.requireAdmin, async (req, res) => {
  try {
 ...
 */
app.get('/api/admin/moderation/flagged-messages', rateLimit({ windowMs: 60000, max: 60 }), authMiddleware.requireAdmin, async (req, res) => {
  try {
 ...
 */
app.get('/api/admin/moderation/user-history/:userId', rateLimit({ windowMs: 60000, max: 60 }), authMiddleware.requireAdmin, async (req, res) => {
  try {
 ...
 */
app.post('/api/admin/moderation/action', rateLimit({ windowMs: 60000, max: 60 }), authMiddleware.requireAdmin, async (req, res) => {
  try {
...
// Duplicate GET /api/admin/stats removed — canonical version with requireAdmin is defined above.
...
// Duplicate POST /api/basket/add removed — canonical version with userBaskets/priceId is defined above.

/**
 * POST /api/basket/remove
 * Body: { priceId }
 * Removes an item from the basket by Stripe price ID.
...
// Duplicate GET /api/basket removed — canonical version with userBaskets is defined above.
