...
test.describe('FREE tier enforcement', () => {
  ...
  test('FREE tier: text message rejected by server', async ({ request }) => {
    ...
    await request.post(`${BASE}/api/join-party`, { data: { partyCode: code, guestId, nickname: 'FreeGuest' } });
...
test.describe('Chat mode enforcement', () => {
  ...
  test.beforeAll(async ({ request }) => {
    ...
    guestId = `g_${uid()}`;
    await request.post(`${BASE}/api/join-party`, { data: { partyCode: code, guestId, nickname: 'ChatGuest' } });
  });
